...
class Model(object):
    ...
    def forward(self, re_embedding, train=True):
        ...
        # print(f'seq_h (batch_size*T*d)', seq_h)
        ...
        # print(f'selected action', selected_actions)
        ...
        selected_action_probabilities = tf.gather(action_probabilities, indices, axis=1)  # π(a|s;θ)
        log_probabilities = tf.math.log(selected_action_probabilities)
        actor_loss = -tf.reduce_mean(log_probabilities * reward)
        if train:
        ...
        return loss, loss_sac, logits, reward, rm, last_id, last_h, seq_h, last, seq, m, coef, b, ma, y1, selected_actions, action_probabilities
...
class GGNN(Model):
    def __init__(self, hidden_size=100, out_size=100, batch_size=300, n_node=None,
        ...
        with tf.variable_scope('actor', reuse=None):
            self.loss_train, self.loss_sac, self.scores_train, self.reward, self.rm, self.last_id, self.last_h, self.seq_h, self.last, self.seq, self.m, self.coef, self.b, self.ma, self.y1, self.action, self.act_prob = self.forward(self.ggnn())
        with tf.variable_scope('actor', reuse=True):
            self.loss_test, self.loss_sac, self.scores_test, self.reward, self.test_rm, self.test_last_id, self.test_last_h, self.test_seq_h, self.test_last, self.test_seq, self.m, self.test_coef, self.test_b, self.test_ma, self.test_y1, self.test_action, self.test_act_prob = self.forward(self.ggnn(), train=False)
    ...
    def ggnn(self):
        ...
        print(f'final state after gru', fin_state)
